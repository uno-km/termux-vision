...
### Fixed
- Enforced `--single-turn` and `--no-conversation` in `SubprocessVLMRuntime` to prevent interactive shell hangs.
- Streamlined prompt formatting to let `llama-cli` apply model-native Jinja chat templates cleanly.
